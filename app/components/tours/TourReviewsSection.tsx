  ...
  return (
    <div className="relative flex flex-col items-center w-[1400px] max-w-[90vw] mt-[100px]">
        ...
        <div className='flex flex-col gap-1 max-md:self-start max-sm:self-center'>
          <div className="text-[1.75rem]">
            ...
            {tour ? (
              ...
              <span className="inline-block w-48 h-8 bg-gray-200 animate-pulse rounded-md align-middle ml-2" />
            )}
          </div>
      ...
      <div className='relative max-sm:w-[100vw] w-[90vw] max-w-[1400px] overflow-visible z-10'>
        ...
        {loading ? (
          /* Show 3 skeletons side-by-side while loading */
          <div className='flex overflow-x-auto pb-12 pt-4 gap-6 px-4 scroll-smooth hide-scrollbar'>
            <ReviewSkeleton />
            <ReviewSkeleton />
            <ReviewSkeleton />
          </div>
          ...
          <div className='flex overflow-x-auto pb-12 pt-4 gap-6 w-full px-4 scroll-smooth hide-scrollbar'>
            {reviews.map(r => (
              <ReviewCard 
                ...
                onReport={(id) => setReportState({isOpen: true, id})} 
                experience={{
                  id: r.id, 
                  user_id: r.user_id,
      ...
      {tour && (
        <>
          ...
          <ReportModal 
            ...
            reviewId={reportState.id} 
            user={user} 
            tableName="tour_review_reports"
            onClose={() => setReportState({isOpen: false, id: null})} 
          />
        </>
      )}
    </div>
