...
const ANDROMEDA_GARDENS_DATA: SiteContent = {
  id: 7,
  name: "Andromeda Gardens",
...
export default function SoupBowlPage() {
  ...
  const {
    ...
    ANDROMEDA_GARDENS_DATA.id,
    ANDROMEDA_GARDENS_DATA.location.lat,
    ANDROMEDA_GARDENS_DATA.location.lng
  );
  ...
  return (
    <main className='flex flex-col min-h-screen text-black items-center overflow-x-hidden'>
      ...
      <HeroSection content={ANDROMEDA_GARDENS_DATA} />

      {/* Decorative Separator */}
      <div className='bg-gray-300 w-full max-w-[80vw] h-[2px] mt-20 mx-auto rounded-full' />

      <SiteFacts facts={ANDROMEDA_GARDENS_DATA.facts}/>

      <InfoSection
        sections={ANDROMEDA_GARDENS_DATA.sections}
        sidebarSlot={
          <>
            <SiteSafety data={ANDROMEDA_GARDENS_DATA.safety}/>
            {ANDROMEDA_GARDENS_DATA.stories && (
              <LocalStories data={ANDROMEDA_GARDENS_DATA.stories} />
            )}
            <SiteQuiz user={user} siteId={ANDROMEDA_GARDENS_DATA.id} />
          </>
        }
      />

      {/* <LocalStories stories={ANDROMEDA_GARDENS_DATA.stories} /> */}

      <GalleryFan items={ANDROMEDA_GARDENS_DATA.gallery} />
      ...
      <ReviewsSection
        ...
        supabase={supabase}
        siteId={ANDROMEDA_GARDENS_DATA.id}
      />
